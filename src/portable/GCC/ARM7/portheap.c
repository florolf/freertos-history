...
#define heapSMALL_BLOCK_SIZE		( ( unsigned portSHORT ) 80 )
#define heapLARGE_BLOCK_SIZE		( portMINIMAL_STACK_SIZE * sizeof( portSTACK_TYPE ) )

/* These are the number of blocks required by the demo application. */
#define heapNUM_SMALL_BLOCKS		( 60 )
#define heapNUM_LARGE_BLOCKS		( 60 )

/* Each block contains the data area and a variable that marks the block as
empty of full. */
typedef struct
{
	unsigned portLONG ucFull;
...
{
	/* The ARM7 demo application does not use any calls to free() so this has

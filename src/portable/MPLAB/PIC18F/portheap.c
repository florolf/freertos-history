...
unsigned portCHAR ucBlock;
	...
	portENTER_CRITICAL();
	{
		if( usSize <= ( unsigned portSHORT ) heapSMALL_BLOCK_SIZE )
		{
			/* Try to find a block in the list of small blocks. */
			for( ucBlock = 0; ucBlock < ( unsigned portCHAR ) heapNUM_SMALL_BLOCKS; ucBlock++ )
			{
				if( xSmallBlocks[ ucBlock ].ucFull == ( unsigned portCHAR ) pdFALSE )
				{
		...
		if( ( usSize <= ( unsigned portSHORT ) heapLARGE_BLOCK_SIZE ) && ( pv == NULL ) )
		{		
			for( ucBlock = 0; ucBlock < ( unsigned portCHAR ) heapNUM_LARGE_BLOCKS; ucBlock++ )
			{	
				if( xLargeBlocks[ ucBlock ].ucFull == ( unsigned portCHAR ) pdFALSE )
				{
...
{
	/* The PIC demo application does not use any calls to free(). */
	pv = NULL;
}
/*-----------------------------------------------------------*/
...
portSHORT sBlock;
	...
	/* Mark all blocks as empty as the RAM is not always initialised. */

	for( sBlock = 0; sBlock < heapNUM_SMALL_BLOCKS; sBlock++ )
	{
		xSmallBlocks[ sBlock ].ucFull = pdFALSE;
	}

	for( sBlock = 0; sBlock < heapNUM_LARGE_BLOCKS; sBlock++ )
	{
		xLargeBlocks[ sBlock ].ucFull = pdFALSE;
	}
}
